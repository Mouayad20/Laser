...
export class AccountProviderComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.accountProviderService
      ...
      .subscribe({
        next: (res: HttpResponse<IAccountProvider[]>) => {
  ...
  trackId(_index: number, item: IAccountProvider): number {
    return item.id!;
  }

  delete(accountProvider: IAccountProvider): void {
    const modalRef = this.modalService.open(AccountProviderDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.accountProvider = accountProvider;
  ...
  protected onSuccess(data: IAccountProvider[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
